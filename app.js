...
// 날짜 기준을 현재일자 그대로 사용 (1일이 되면 바로 다음 달로 인식)
...
function autoRegisterPastedStores(grid, schema, account){
  // 폐업 매장 부활 차단 로직 적용: 전월, 전년동월 데이터 복사 시에는 신규 매장을 자동 생성하지 않음
  if (state.entryPeriod === '전월' || state.entryPeriod === '전년동월') return;
...
function finalizeMonthClose(closedMonthKey){
  ...
  // 자동마감 시 '당월누적'에 있던 모든 데이터를 100% 동일하게 '전월'로 복사
  STORES.forEach(store => {
    const rec = SALES['당월누적']?.[store.name];
